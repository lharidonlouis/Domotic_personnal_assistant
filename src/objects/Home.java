...
public class Home implements DbSetter {
	 ...
	 * 	Objet à utiliser
	 * <p>
	 * Fonction permettant de réaliser une action en récupérant dans
	 * la Map d'objets l'élément de clef objToUse, et en lui faisant
	 * faire l'action toDo
	 * </p>
	 */
	...
	/**
	 * Variable du chemin du fichier log
	 */
	 ...
	 * <p>
	 * Parcours le fichier log. Pour chaque objet stocke ses action et ses clock dans un ObjToAutomate.
	 * </p>
	 */
				...
				if (readLine[5].equals(clock.getWeek().toString())) {
					...
					for (ObjToAutomate i : list) {
						if (i.getName().equals(readLine[0])) {
							...
							tempclk.set(Integer.parseInt(readLine[2]), Integer.parseInt(readLine[3]),
									Integer.parseInt(readLine[4]), Integer.parseInt(readLine[5]));
	...
	/**
	 * Fonction Automation pour la semaine passée
	 * <p>
	 * Parcours le fichier log. Pour chaque objet stocke ses action et ses clock dans un ObjToAutomate.
	 * </p>
	 */
	...
	/**
	 * Mise à jour finale de l'automation
	 * <p>
	 * Cette fonction permet de comparer le fichier d'automationd de base au
	 * fichier provisoire afin de recalculer si nécessaire les nouvelles plages
	 * horraires ou en rajouter
	 * </p>
	 */
						...
						if ((readLine1[0].equals(readLine2[0])) && (readLine1[1].equals(readLine2[1]))) {
							...
							if (temp1.inrange(120, temp2)) {
								...
								try {
									...
									// Create record
									String[] record = (readLine1[0] + "," + readLine1[1] + "," + temp1.getMinute() + ","
											+ temp1.getHour()).split(",");
